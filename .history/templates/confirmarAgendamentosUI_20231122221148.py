...
class ConfirmarAgendamentoUI:
    ...
    def confirmar_agendamento():
        ...
        if st.button("Confirmar"):
            ...
            time.sleep(2)
            st.rerun()
        if st.button("Não confirmar"):    
            View.agenda_atualizar(horario.get_id(), horario.get_data(), False, horario.get_id_cliente(), horario.get_id_servico())
            st.error("Agendamento não confirmado")
            time.sleep(2)
            st.rerun()